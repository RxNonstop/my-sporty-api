...
class MiembrosEquipoController {
    ...
    static async store(req, res) {
        try {
            ...
            const [existente] = await db.query('SELECT * FROM miembrosequipo WHERE usuario_id = ? AND equipo_id = ?', [data.usuario_id, data.equipo_id]);
            
            if (existente.length > 0) {
                if (existente[0].activo == 1) return res.status(409).json({ status: 409, message: 'El usuario ya es miembro' });
                else {
                    await db.query('UPDATE miembrosequipo SET activo = 1, rol_usuario = ? WHERE id = ?', [data.rol_usuario || 'jugador', existente[0].id]);
                    return res.status(201).json({ status: 201, message: 'Agregado' });
                }
            }

            await db.query('INSERT INTO miembrosequipo (usuario_id, equipo_id, rol_usuario, activo) VALUES (?, ?, ?, 1)', [data.usuario_id, data.equipo_id, data.rol_usuario || 'jugador']);
            return res.status(201).json({ status: 201, message: 'Agregado' });
        } catch (error) {
            return res.status(500).json({ status: 500, message: 'Error', data: { detalles: error.message } });
        }
    ...
    static async update(req, res) {
        try {
            ...
            const [result] = await db.query('UPDATE miembrosequipo SET rol_usuario = ? WHERE usuario_id = ? AND equipo_id = ? AND activo = 1', [data.rol_usuario, data.usuario_id, data.equipo_id]);
    ...
    static async delete(req, res) {
        try {
            ...
            const [result] = await db.query('UPDATE miembrosequipo SET activo = 0 WHERE usuario_id = ? AND equipo_id = ? AND activo = 1', [data.usuario_id, data.equipo_id]);
